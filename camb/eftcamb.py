...
class EFTCAMB(F2003Class):
    ...
    def get_eft_functions(self, camb_results, a):
        results = EFTCAMB_timestep_cache()
        EFTCAMB_eftfunctions(byref(camb_results),
                             byref(c_double(a)), # change to Look; pay attention to designer models
                             byref(results)
                             )
        fields = results.variables()
        results = np.ctypeslib.as_array(results)
        return fields, results
...
class EFTCAMB_parameter_cache(F2003Class):
    ...
    _fields_ = [
        # 0a) Horndeski module: EDE shooting parameters
        ("maxfrac_hdsk", c_double),
        ("z_maxfrac_hdsk", c_double),
        # 0b) Horndeski module: designer approach products
        # ("designer_hdsk_phi", AllocatableArrayDouble),
        # ("designer_hdsk_V", AllocatableArrayDouble),
