    ...
    ! EFTCAMB MOD START: check QSA
    if (CP%EFTCAMB%EFTflag/=0 .and. EV%EFTCAMBactive .and. CP%EFTCAMB%EFT_check_QSA .and. EV%eft_cache%a>CP%EFTCAMB%EFT_QSA_time .and. EV%eft_cache%k>CP%EFTCAMB%EFT_QSA_k) then
        if (abs(EV%eft_cache%mu/EV%eft_cache%QS_mu_gen - 1._dl)>CP%EFTCAMB%EFT_QSA_threshold .or. abs(EV%eft_cache%sigma_eft/EV%eft_cache%QS_sigma_gen - 1._dl)>CP%EFTCAMB%EFT_QSA_threshold) then
            write(*,*) "QSA broke at k=", EV%eft_cache%k, "a=", EV%eft_cache%a
            write(*,*) "mu_QSA_gen=", EV%eft_cache%QS_mu_gen, "mu_QSA=", EV%eft_cache%QS_mu, "mu=", EV%eft_cache%mu
            write(*,*) "sigma_QSA_gen=", EV%eft_cache%QS_sigma_gen, "sigma_QSA=", EV%eft_cache%QS_sigma, "sigma=", EV%eft_cache%sigma_eft
            call GlobalError('QSA check is on. Rejecting possibly healthy model due to QSA breaking down.', error_evolution)
        end if
    end if
    ! EFTCAMB MOD END.
