...
module linear_interpolation_1D
    ...
    contains
        ...
        procedure :: initialize_integration => LinearIntepolateFunction1DInitIntegration    !< subroutine that initializes the integration vector. Jacobian is not supportted now. by Gen
...
contains
    ...
    subroutine LinearIntepolateFunction1DInitDerivatives( self, jacobian )
        ...
        !compute first four derivatives
        call spline5ders( self%x, self%y, self%x*0.8, dummy, self%yp, self%ypp, self%yppp, self%ypppp )
        ! write(*,*) "---------------------------"
        ! do i=1, self%num_points
        !     write(*,*) self%x(i), self%y(i), dummy(i), self%yp(i), self%ypp(i), self%yppp(i), self%ypppp(i)
        ! end do
        ! write(*,*) "==========================="
    ...
    subroutine LinearIntepolateFunction1DInitIntegration( self, jacobian )
        ...
        if ( present(jacobian) ) then
            ...
            write(*,*) 'LinearIntepolateFunction1DInitIntegration does not support jacobian.'
