...
module EFTCAMB_main
    ...
    use EFTCAMB_Reparametrized_Horndeski_alphaDE
...
contains
    ...
    subroutine read_EFTCAMB_flags_from_file( self, Ini )
        ...
        !) QSA checks:
        self%EFT_check_QSA              = Ini%Read_Logical( 'EFT_check_QSA'    , .false. )
        self%EFT_QSA_threshold          = Ini%Read_Double( 'EFT_QSA_threshold', 0.1_dl )
        self%EFT_QSA_time               = Ini%Read_Double( 'EFT_QSA_time', 1._dl )
        self%EFT_QSA_k                  = Ini%Read_Double( 'EFT_QSA_k', 1._dl )
        ...
        if ( self%EFTCAMB_use_background .and. ( self%EFTflag /= 4 .or. self%FullMappingEFTmodel /= 0) ) then
            write(*,*) "EFTCAMB Error: EFTCAMB_use_background=True currently only supported by the Horndeski module (EFTflag=4, FullMappingEFTmodel=0)"
            call MpiStop("EFTCAMB error")
        end if
        self%EFTCAMB_evolve_delta_phi         = Ini%Read_Logical( 'EFTCAMB_evolve_delta_phi', .false. )
        if ( self%EFTCAMB_evolve_delta_phi .and. ( self%EFTflag /= 4 .or. self%FullMappingEFTmodel /= 0) ) then
            write(*,*) 'EFTCAMB Error: EFTCAMB_evolve_delta_phi=True currently only supported by the Horndeski module (EFTflag = 4 and FullMappingEFTmodel=0).'
            call MpiStop('EFTCAMB error')
        end if
        ...
        select case ( self%EFTflag )
            ...
            case (3)     ! Designer mapping EFT:
                ...
                select case ( self%DesignerEFTmodel )
                    ...
                    case(4)
                        allocate( EFTCAMB_RPH_alphaDE::self%model )
                        call self%model%init( 'RPHalphaDE', 'RPHalphaDE' )
                    case default
                        if (self%EFTCAMB_feedback_level > 0) then
                            write(*,'(a,I3)') 'No model corresponding to EFTFlag =', self%EFTflag
                            ...
                            write(*,'(a)')    'DesignerEFTmodel=4  reparametrized Horndeski with alpha_i = c_i * Omega_DE'
                        end if
                        eft_error = 1
                        return
                end select
