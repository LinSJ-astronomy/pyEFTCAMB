...
module EFTCAMB_abstract_model
    ...
    contains
        ...
        procedure :: compute_DeltaTau            => EFTCAMBModelComputeDeltaTau                           !< function that computes the conformal time difference between two scale factor
        procedure :: compute_DeltaCosmicT        => EFTCAMBModelComputeDeltaCosmicT                       !< function that computes the cosmic time difference between two scale factor
        procedure :: compute_SoundHorizon        => EFTCAMBModelComputeSoundHorizon                       !< function that computes r_s at given scale factor
        ...
        procedure :: compute_QS_Mu_Sigma         => EFTCAMBModelComputeQSMuSigma                          !< function that computes the quasi-static expression of Sigma.
...
contains
    ...
    subroutine EFTCAMBModelComputeQSMu( self, a, k,  eft_par_cache, eft_cache )
      ...
      eft_cache%QS_mu = (2*(2*(1 + eft_cache%EFTOmegaV)*eft_cache%EFTc*(k**2 + 3*eft_cache%adotoa**2 &
    ...
    end subroutine EFTCAMBModelComputeQSMu

    ! ---------------------------------------------------------------------------------------------

    subroutine EFTCAMBModelComputeQSSigma( self, a, k,  eft_par_cache, eft_cache )
      ...
      eft_cache%QS_sigma =(2*(2*(1 + eft_cache%EFTOmegaV)*eft_cache%EFTc*(k**2 + 3*eft_cache%adotoa**2 &
            &- 3*eft_cache%Hdot) +a*(-36*eft_cache%adotoa**4*(1 + eft_cache%EFTOmegaV)*eft_cache%EFTOmegaP &
            &- 3*eft_cache%adotoa**3*eft_par_cache%h0_Mpc*(1 + eft_cache%EFTOmegaV)*(eft_cache%EFTGamma2V &
            &+ a*eft_cache%EFTGamma2P) +2*k**2*eft_cache%adotoa**2*(2*eft_cache%EFTGamma5V*eft_cache%EFTOmegaP &
            &+ a*eft_cache%EFTOmegaP**2 +2*(1 + eft_cache%EFTOmegaV + a*eft_cache%EFTOmegaP)*eft_cache%EFTGamma5P) &
   ...
   end subroutine EFTCAMBModelComputeQSSigma


   ! ---------------------------------------------------------------------------------------------

   subroutine EFTCAMBModelComputeQSMuSigma( self, a, k,  eft_par_cache, eft_cache ) ! minsup
     ...
     real(dl):: A1, A2, A3, B1, B2, B3, C1, C2, C3, M2, R0dot       !< temp variables
     real(dl):: gamma

     R0dot = (6/a**3)*(eft_cache%Hdotdot-2*eft_cache%adotoa*(eft_cache%adotoa**2))

     A1 = 2*(1+eft_cache%EFTOmegaV) + 4*eft_cache%EFTGamma5V
     A2 =  -eft_cache%EFTOmegaP*eft_cache%adotoa - eft_par_cache%h0_Mpc*eft_cache%EFTGamma2V + 2*eft_cache%adotoa*eft_cache%EFTGamma4V/a &
      & + 4*eft_cache%adotoa*eft_cache%EFTGamma5V/a
     A3 = -8*eft_cache%EFTGamma6V

     B1 = -1 - 2*eft_cache%EFTGamma5V/(1+eft_cache%EFTOmegaV)
     B2 = 1
     B3 = (-eft_cache%EFTOmegaP*eft_cache%adotoa + eft_cache%adotoa*(eft_cache%EFTGamma4V/a + eft_cache%EFTGamma4P)) &
      &  /(1+eft_cache%EFTOmegaV)

     C1 = eft_cache%EFTOmegaP*eft_cache%adotoa + 2*eft_cache%adotoa*(eft_cache%EFTGamma5V/a+eft_cache%EFTGamma5P)
     C2 = -eft_cache%EFTOmegaP*eft_cache%adotoa/2 - eft_cache%eftgamma2V*eft_par_cache%h0_Mpc/2 - (1/(2*a))*eft_cache%adotoa &
      & *(3*eft_cache%EFTGamma3V + eft_cache%EFTGamma4V - 4*eft_cache%EFTGamma5V)
     C3 = eft_cache%EFTc/a**2 - eft_par_cache%h0_Mpc*eft_cache%adotoa*(eft_cache%eftgamma2V/a+eft_cache%eftgamma2P)/2 &
      & + eft_cache%EFTGamma3V*(k**2/2 - 3 *(eft_cache%Hdot-eft_cache%adotoa**2))/(a**2) &
      & + eft_cache%EFTGamma4V*(k**2/2 -    (eft_cache%Hdot-eft_cache%adotoa**2))/(a**2) &
      & + 2*(eft_cache%Hdot*eft_cache%EFTGamma5V/a**2 + eft_cache%adotoa**2*eft_cache%EFTGamma5P/a)

     M2 = eft_cache%EFTOmegaP*eft_cache%adotoa*R0dot/4 - 3*eft_cache%EFTc*(eft_cache%Hdot-eft_cache%adotoa**2)/a**4 &
      & + 1.5*(eft_par_cache%h0_Mpc/a**2)*((eft_cache%Hdot-eft_cache%adotoa**2)*eft_cache%adotoa*eft_cache%EFTGamma2P &
      & + (eft_cache%Hdotdot - eft_cache%adotoa*eft_cache%Hdot - eft_cache%adotoa**3)*eft_cache%EFTGamma2V/a) &
      & + 1.5*((eft_cache%Hdot-eft_cache%adotoa**2)**2/a**4)*(eft_cache%EFTGamma4V + 3*eft_cache%EFTGamma3V)

     eft_cache%QS_mu_gen = 2*(B2*C3-C1*B3+(a**2/k**2)*B2*M2) &
      & /(A1*(B3*C2-B1*C3-B1*M2*(a**2/k**2)) + A2*(B1*C1-B2*C2) + A3*(B2*C3-B3*C1+B2*M2*(a**2/k**2)))

     gamma = (B3*C2-B1*C3-(a**2/k**2)*B1*M2)/(B2*C3-C1*B3+(a**2/k**2)*B2*M2)

     eft_cache%QS_sigma_gen = 0.5*eft_cache%QS_mu_gen*(1+gamma)

    end subroutine EFTCAMBModelComputeQSMuSigma
    ...
    function EFTCAMBModelComputeDeltaTau( self, a1, a2 )
      ...
      real(dl)                                      :: EFTCAMBModelComputeDeltaTau !< the output tau

      write(*,*) "EFTCAMB_use_background = True"
      write(*,*) "EFTCAMBModelComputeDeltaTau currently only supported by the Horndeski module (EFTflag=4, FullMappingEFTmodel=0)"
      call MpiStop("EFTCAMB error")

    end function EFTCAMBModelComputeDeltaTau

    function EFTCAMBModelComputeDeltaCosmicT( self, a1, a2 )
      ...
      real(dl)                                      :: EFTCAMBModelComputeDeltaCosmicT !< the output t

      write(*,*) "EFTCAMB_use_background = True"
      write(*,*) "EFTCAMBModelComputeDeltaCosmicT currently only supported by the Horndeski module (EFTflag=4, FullMappingEFTmodel=0)"
      call MpiStop("EFTCAMB error")

    end function EFTCAMBModelComputeDeltaCosmicT

    function EFTCAMBModelComputeSoundHorizon( self, a )
      ...
      real(dl)                                      :: EFTCAMBModelComputeSoundHorizon !< the output r_s

      write(*,*) "EFTCAMB_use_background = True"
      write(*,*) "EFTCAMBModelComputeSoundHorizon currently only supported by the Horndeski module (EFTflag=4, FullMappingEFTmodel=0)"
      call MpiStop("EFTCAMB error")

    end function EFTCAMBModelComputeSoundHorizon
