...
contains
  ...
  subroutine EFTCAMB_GetEFTFunctions( State, a, timestep_cache)
    Type(CAMBdata)                :: State
    real(dl), intent(in)          :: a
    type(TEFTCAMB_timestep_cache) :: timestep_cache

    call State%CP%EFTCAMB%model%compute_adotoa( a, State%CP%eft_par_cache, timestep_cache )
    call State%CP%EFTCAMB%model%compute_H_derivs( a, State%CP%eft_par_cache, timestep_cache )
    call State%CP%EFTCAMB%model%compute_rhoQPQ( a, State%CP%eft_par_cache, timestep_cache )
    call State%CP%EFTCAMB%model%compute_background_EFT_functions( a, State%CP%eft_par_cache, timestep_cache )
    call State%CP%EFTCAMB%model%compute_secondorder_EFT_functions( a, State%CP%eft_par_cache, timestep_cache )
