...
module EFTCAMB_full_Beyond_Horndeski
    ...
    contains
       ...
       ! procedure :: compute_dtauda                    => EFTCAMBBeyondHorndeskiComputeDtauda    !< function that computes dtauda = 1/sqrt(a^2H^2).
       ! procedure :: compute_adotoa                    => EFTCAMBBeyondHorndeskiComputeAdotoa            !< subroutine that computes adotoa = H and its two derivatives wrt conformal time.
       ! procedure :: compute_H_derivs                  => EFTCAMBBeyondHorndeskiComputeHubbleDer         !< subroutine that computes the two derivatives wrt conformal time of H.
...
contains
       ...
       ! some feedback:
        ...
        if (self%debug_flag) then
          ...
          do j =1, self%fun_x1%num_points
            ...
            write(555, '(20E15.5)')1._dl/aj-1._dl, sqrt(H2), (grhorad/aj**4)/(3._dl*H2)&
    ...
    subroutine EFTCAMBBeyondHorndeskiBackgroundEFTFunctions( self, a, eft_par_cache, eft_cache )
        ...
        real(dl) :: OmegaRad,OmegaRadprime,h,hprime,hpprime, grhom, grhorad, H2, qspprime
        ...
        eft_cache%EFTc         = eft_cache%adotoa**2.*(h*x4p/10._dl-x4p/10._dl + x4pp/10._dl + (1._dl/10._dl)*(5._dl*(3._dl*x1+x2-epsilon*x3)-(3._dl+8._dl*epsilon)*x4) + (x1+x2+x3+x4)*3._dl/2._dl)

        eft_cache%EFTcdot      = eft_cache%EFTc*(2._dl*eft_cache%Hdot/eft_cache%adotoa-2._dl*eft_cache%adotoa)  + eft_cache%adotoa**3 * ( h*x4pp/10._dl+hprime*x4p/10._dl&
                               & -x4pp/10._dl + x4ppp/10._dl + (1._dl/10._dl)*(5._dl*(3._dl*x1p+x2p-epsilon*x3p-epsilon_p*x3)&
                               &- 8._dl*epsilon_p*x4-x4p*(3._dl+8._dl*epsilon)) + (x1p+x2p+x3p+x4p)*3._dl/2._dl)

        eft_cache%EFTLambda    = eft_cache%adotoa**2.*((3._dl+ h)* x4p/5._dl + 6._dl*x4/5._dl -x4p/5._dl + x4pp/5._dl + 3._dl*x1+x2-epsilon*x3 -(3._dl+8._dl*epsilon)*x4/5._dl)

        eft_cache%EFTLambdadot = eft_cache%EFTLambda*(2._dl*eft_cache%Hdot/eft_cache%adotoa-2._dl*eft_cache%adotoa)  + eft_cache%adotoa**3 &
                               & * ((3._dl+ h)* x4pp/5._dl+hprime* x4p/5._dl+6._dl*x4p/5._dl -x4pp/5._dl + x4ppp/5._dl + 3._dl*x1p + x2p &
        ...
        if ( x4 == 0._dl ) then
           ...
           eft_cache%EFTLambdadotdot = 1._dl/3._dl *(-5._dl * eft_cache%adotoa**2._dl * eft_cache%Hdot *( x3*hprime + x3p*h - 2._dl*h*x3 - 9._dl*x1p - 3._dl*x2p - 2._dl*x3p + x3pp + 18._dl*x1 + 6._dl*x2)           &
                                       + eft_cache%adotoa**4._dl *(-x3*hpprime + 4._dl*x3*hprime - 2._dl*hprime*x3p + 4._dl*h*x3p - h*x3pp - 4._dl*h*x3 - 36._dl*x1p - 12._dl*x2p - 4._dl*x3p + 9._dl*x2pp            &
                                       + 3._dl*x2pp + 4._dl*x3pp + 36._dl*x1 + 12._dl*x2 - x3ppp) + 2._dl*eft_cache%adotoa*eft_cache%Hdotdot*(-h*x3 - x3p + 9._dl*x1 + 3._dl*x2) + 2._dl*eft_cache%Hdot**2._dl*(      &
    ...
    subroutine EFTCAMBBeyondHorndeskiSecondOrderEFTFunctions( self, a, eft_par_cache, eft_cache )
        ...
        real(dl) :: mu, x,h,hprime,hpprime,qs,qsprime,qspprime
        integer  :: ind
        ...
        eft_cache%EFTGamma1V  = (2*eft_cache%adotoa**2*x2)/(a**2*eft_par_cache%h0_Mpc**2) +(eft_cache%adotoa**2*((3 + h/3.)*x3 + x3p/3.))/(4.*a**2*eft_par_cache%h0_Mpc**2) + &
                                (eft_cache%adotoa**2*(6*x4 + (3*x4p)/4. - (h*x4p)/4. -x4pp/4.))/(5.*a**2*eft_par_cache%h0_Mpc**2)
        eft_cache%EFTGamma1P  = (2*eft_cache%Hdot*(120*x2 + 5*(9 + h)*x3 + 72*x4 +5*x3p + 9*x4p - 3*h*x4p -3*x4pp) + eft_cache%adotoa**2*(-240*x2 - 144*x4 -5*x3*(18 + 2*h - hprime) &
                                 + 120*x2p +35*x3p + 5*h*x3p + 54*x4p +6*h*x4p - 3*hprime*x4p +5*x3pp + 15*x4pp - 3*h*x4pp -3*x4ppp))/(60.*a**3*eft_par_cache%h0_Mpc**2)
        eft_cache%EFTGamma2V  = -(eft_cache%adotoa*x3)/(a*eft_par_cache%h0_Mpc) + (eft_cache%adotoa*(-8*x4 + x4p))/(5.*a*eft_par_cache%h0_Mpc)
        eft_cache%EFTGamma2P  = -(eft_cache%Hdot*(5*x3 + 8*x4 - x4p) -eft_cache%adotoa**2*(5*x3 + 8*x4 - 5*x3p - 9*x4p +x4pp))/(5.*a**2*eft_cache%adotoa*eft_par_cache%h0_Mpc)
        ...
        if (x4 == 0._dl) then
          ...
          eft_cache%EFTGamma2PP = (x3*eft_cache%Hdot**2._dl - x3*eft_cache%adotoa*eft_cache%Hdotdot + eft_cache%adotoa**4._dl*(3._dl*x3p-x3pp-2._dl*x3) + &
                                   eft_cache%adotoa**2._dl*eft_cache%Hdot*(3._dl*x3 - 2._dl*x3p))/(a**3._dl*eft_par_cache%h0_Mpc*eft_cache%adotoa**3._dl)
                                  
          eft_cache%EFTGamma2PPP= (3._dl*eft_cache%adotoa**2._dl*eft_cache%Hdot**2._dl*(x3p-2._dl*x3) + 4._dl*eft_cache%adotoa*eft_cache%Hdot*eft_cache%Hdotdot*x3 + 3._dl*eft_cache%adotoa**3._dl*eft_cache%Hdotdot*(2._dl*x3 - &
                                  x3p) - eft_cache%adotoa**2._dl*eft_cache%Hdotdotdot*x3 + eft_cache%adotoa**4._dl*eft_cache%Hdot*(-11._dl*x3+12._dl*x3p-3._dl*x3pp) + eft_cache%adotoa**6._dl*(6._dl*x3 - 11._dl*x3p            &
                                  +6._dl*x3pp - x3ppp) - eft_cache%Hdot**3._dl*x3)/(a**4._dl*eft_par_cache%h0_Mpc*eft_cache%adotoa**5._dl) 
    ...
    ! ---------------------------------------------------------------------------------------------
    function EFTCAMBBeyondHorndeskiComputeDtauda( self, a, eft_par_cache, eft_cache )
        ...
        real(dl) :: EFTCAMBBeyondHorndeskiComputeDtauda ,temp, temp1, temp2                   !< the output dtauda
        ...
        if (a*eft_cache%adotoa==0._dl .or. x <self%x_initial) then
          EFTCAMBBeyondHorndeskiComputeDtauda = 1._dl/sqrt( eft_cache%grhoa2/3._dl )
        else
          EFTCAMBBeyondHorndeskiComputeDtauda = 1._dl/(a*eft_cache%adotoa)
        end if
        write(*,*)x,self%x_initial
    end function EFTCAMBBeyondHorndeskiComputeDtauda

    ! ---------------------------------------------------------------------------------------------
    !> Subroutine that computes adotoa = H.
    subroutine EFTCAMBBeyondHorndeskiComputeAdotoa( self, a, eft_par_cache, eft_cache )
        ...
        real(dl) :: x, mu,mu_0, H2, grhom,grhorad, x1,x2,x3,x4
        integer  :: ind,ind_0
        ...
        eft_cache%adotoa = sqrt(H2)*a
        
        write(*,*)1._dl - x1-x2-x3,exp(x),sqrt(H2)*a

    end subroutine EFTCAMBBeyondHorndeskiComputeAdotoa

    ! ---------------------------------------------------------------------------------------------
    !> Subroutine that computes the two derivatives wrt conformal time of H.
    subroutine EFTCAMBBeyondHorndeskiComputeHubbleDer( self, a, eft_par_cache, eft_cache )
        ...
        real(dl) :: x, mu, temp,grhog_t, h, x1,x2,x3,x4,OmegaRad,OmegaRadprime,OmegaRadprimeprime, hprime, x1p,x2p,x3p,x4p, H2, grhom, grhorad
        real(dl) :: hpprime, x1pp,x2pp,x3pp,x4pp, qs,qsprime,qspprime
        integer  :: ind
        ...
        x3 = self%fun_x3%value(x, index=ind, coeff=mu)
        x4 = self%fun_x4%value(x, index=ind, coeff=mu)
        x1p = self%fun_x1%first_derivative(x, index=ind, coeff=mu)
        x2p = self%fun_x2%first_derivative(x, index=ind, coeff=mu)
        x3p = self%fun_x3%first_derivative(x, index=ind, coeff=mu)
        x4p = self%fun_x4%second_derivative(x, index=ind, coeff=mu)
        x1p = self%fun_x1%second_derivative(x, index=ind, coeff=mu)
        x2p = self%fun_x2%second_derivative(x, index=ind, coeff=mu)
        x3p = self%fun_x3%second_derivative(x, index=ind, coeff=mu)
        x4p = self%fun_x4%second_derivative(x, index=ind, coeff=mu)
        ...
        OmegaRadprimeprime = -2._dl*OmegaRadprime*(2._dl +h) -2._dl*OmegaRad*hprime
        grhog_t = eft_par_cache%grhog/a**2._dl
        eft_cache%Hdot = eft_cache%adotoa**2._dl*( 1._dl+h )
        eft_cache%Hdotdot = 2._dl*eft_cache%adotoa*eft_cache%Hdot*( 1._dl+h )+eft_cache%adotoa**3._dl*hprime
        ...
        if (x4 == 0._dl) then
         ...
         eft_cache%Hdotdotdot = 2._dl*eft_cache%Hdot**2._dl*(1+h) + 2._dl * eft_cache%adotoa*eft_cache%Hdotdot*(1+h) + 5._dl*eft_cache%adotoa**2._dl *eft_cache%Hdot*hprime + eft_cache%adotoa**4._dl*hpprime
        end if

    end subroutine EFTCAMBBeyondHorndeskiComputeHubbleDer
