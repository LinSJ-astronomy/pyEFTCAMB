...
contains
    ...
    subroutine EFTCAMBRPHReadModelSelectionFromFile( self, Ini, eft_error )
        ...
        if ( self%RPHalphaMmodel_ODE > 0 .or. self%RPHmassPmodel_ODE > 0 .or. self%RPHkineticitymodel_ODE > 0 .or. self%RPHbraidingmodel_ODE > 0 .or. self%RPHtensormodel_ODE > 0 ) then
    ...
    subroutine EFTCAMBRPHFeedback( self, print_params )
        ...
        if ( self%RPHkineticitymodel_ODE /= 0 ) write(*,'(a,I3)') '   RPHkineticitymodel_ODE =', self%RPHkineticitymodel
        if ( self%RPHbraidingmodel_ODE   /= 0 ) write(*,'(a,I3)') '   RPHbraidingmodel_ODE   =', self%RPHbraidingmodel
        if ( self%RPHtensormodel_ODE     /= 0 ) write(*,'(a,I3)') '   RPHtensormodel_ODE     =', self%RPHtensormodel
    ...
    subroutine EFTCAMBRPHInitBackground( self, params_cache, feedback_level, success, outroot )
        ...
        if ( self%RPHusealphaM ) call self%RPH_M%initialize( self%designer_num_points, self%x_initial, self%x_final )
    ...
    contains
        ...
        if ( self%RPHusealphaM ) ydot(2) = self%RPH_alphaM%value(a) + self%RPH_alphaM_ODE%value(Ode)
    ...
    subroutine EFTCAMBRPHBackgroundEFTFunctions( self, a, eft_par_cache, eft_cache )
        ...
        real(dl) :: aM, daM, d2aM, d3aM, d4aM, Mp1
        ...
        if ( self%RPHusealphaM ) then
            ...
            d3aM        = self%RPH_alphaM%third_derivative(a) + (d3Ode*self%RPH_alphaM_ODE%first_derivative(Ode) + 3*d2Ode*dOde*self%RPH_alphaM_ODE%second_derivative(Ode) + dOde**3*self%RPH_alphaM_ODE%third_derivative(Ode))
            d4aM        = self%RPH_alphaM%fourth_derivative(a) + (d4Ode*self%RPH_alphaM_ODE%first_derivative(Ode) + 3._dl*d2Ode**2*self%RPH_alphaM_ODE%second_derivative(Ode) + 4._dl*d3Ode*dOde*self%RPH_alphaM_ODE%second_derivative(Ode) + 6._dl*d2Ode*dOde**2*self%RPH_alphaM_ODE%third_derivative(Ode) + dOde**4*self%RPH_alphaM_ODE%fourth_derivative(Ode))
            x = log(a)
            call self%RPH_M%precompute(x, ind, mu)
        ...
        RPH_AT_PPP             = self%RPH_Tensor%third_derivative(a) + (d3Ode*self%RPH_Tensor_ODE%first_derivative(Ode) + 3*d2Ode*dOde*self%RPH_Tensor_ODE%second_derivative(Ode) + dOde**3*self%RPH_Tensor_ODE%third_derivative(Ode))
        ...
        eft_cache%EFTcdotdot   = (-(a**4*eft_cache%EFTOmegaPPPP*eft_cache%adotoa**4) - 6._dl*a**3*eft_cache%EFTOmegaPPP*eft_cache%adotoa**2*eft_cache%Hdot + a**2*eft_cache%EFTOmegaPP*(eft_cache%adotoa**4 + 4._dl*eft_cache%adotoa**2*eft_cache%Hdot - 3._dl*eft_cache%Hdot**2 - 4._dl*eft_cache%adotoa*eft_cache%Hdotdot) + a*eft_cache%EFTOmegaP*(-5._dl*eft_cache%adotoa**4 + 10._dl*eft_cache%adotoa**2*eft_cache%Hdot + eft_cache%Hdot**2 - eft_cache%Hdotdotdot) + 2._dl*(4._dl*eft_cache%adotoa**4 - 14._dl*eft_cache%adotoa**2*eft_cache%Hdot + 4._dl*eft_cache%Hdot**2 + 6._dl*eft_cache%adotoa*eft_cache%Hdotdot - eft_cache%Hdotdotdot)*eft_cache%EFTOmegaV + grhov*eft_cache%Hdot*(a*self%RPH_wDE%first_derivative(a) - 3._dl*(1 + self%RPH_wDE%value(a))**2) + grhov*eft_cache%adotoa**2*(a**2*self%RPH_wDE%second_derivative(a) + 9._dl*(1 + self%RPH_wDE%value(a))**3 - a*self%RPH_wDE%first_derivative(a)*(8._dl + 9._dl*self%RPH_wDE%value(a))))/2._dl
    ...
    function EFTCAMBRPHComputeDtauda( self, a, eft_par_cache, eft_cache )
        ...
        EFTCAMBRPHComputeDtauda = sqrt(3._dl/temp)
    ...
    subroutine EFTCAMBRPHComputeHubbleDer( self, a, eft_par_cache, eft_cache )
        ...
        if ( log(a) < self%rhov_t%x_initial ) then
        ...
        eft_cache%Hdotdotdot = (-3._dl*eft_cache%gpinudotdot_tot - 9._dl*eft_cache%gpinudot_tot*eft_cache%adotoa + eft_cache%Hdot*(-3._dl*eft_cache%gpinu_tot + eft_cache%grhob_t +eft_cache%grhoc_t + eft_cache%grhonu_tot + 4._dl*(eft_cache%grhor_t + eft_cache%grhog_t) + eft_cache%grhov_t - 3._dl*a*self%RPH_wDE%first_derivative(a)*eft_cache%grhov_t + 6._dl*eft_cache%grhov_t*self%RPH_wDE%value(a) + 9._dl*eft_cache%grhov_t*self%RPH_wDE%value(a)**2) - eft_cache%adotoa**2*(9._dl*eft_cache%gpinu_tot + eft_cache%grhob_t +eft_cache%grhoc_t + eft_cache%grhonu_tot + 8._dl*(eft_cache%grhor_t + eft_cache%grhog_t) + eft_cache%grhov_t + 3*a**2*self%RPH_wDE%second_derivative(a)*eft_cache%grhov_t - 6*a*self%RPH_wDE%first_derivative(a)*eft_cache%grhov_t + 9._dl*eft_cache%grhov_t*self%RPH_wDE%value(a) - 27._dl*a*self%RPH_wDE%first_derivative(a)*eft_cache%grhov_t*self%RPH_wDE%value(a) + 27._dl*eft_cache%grhov_t*self%RPH_wDE%value(a)**2 + 27._dl*eft_cache%grhov_t*self%RPH_wDE%value(a)**3))/6._dl
