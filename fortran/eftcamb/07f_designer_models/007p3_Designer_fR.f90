...
contains
    ...
    subroutine EFTCAMBDesignerFRAllocateModelSelection( self, Ini, eft_error )
        ...
        select case ( self%EFTwDE )
            ...
            case(5)
                ...
                call self%DesfRwDE%set_name( 'EFTw', 'w' )
            case default
                call allocate_parametrized_1D_function( self%DesfRwDE, self%EFTwDE, 'EFTw', 'w', eft_error, temp_feedback )
                if ( eft_error == 1 ) return
        end select

        ! additional initialization of the function:
        call self%DesfRwDE%init_func_from_file( Ini, eft_error )
    ...
    subroutine EFTCAMBDesignerFRInitModelParametersFromFile( self, Ini, eft_error )
        ...
        call self%DesfRwDE%init_from_file( Ini, eft_error )
